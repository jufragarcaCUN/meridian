...
<p style="color: white; margin: 5px 0 0 0; font-size: 11px;">Pipeline Multi-Agente MMM</p>
    ...
    # ===== VERIFICACIÓN DE DATAFRAME CARGADO =====
    # Mostrar información del DataFrame en el sidebar para confirmación
    st.sidebar.write("---")
    st.sidebar.subheader("📊 DataFrame Cargado")

    # Detector de cambio de archivo usando session_state
    if "archivo_anterior" not in st.session_state:
        st.session_state.archivo_anterior = archivo_seleccionado

    # CORREGIDO: Solo mostrar mensaje, NO llamar a st.rerun()
    if archivo_seleccionado != st.session_state.archivo_anterior:
        st.sidebar.success(
            f"🔄 Cambio detectado: {st.session_state.archivo_anterior} → {archivo_seleccionado}"
        )
        st.session_state.archivo_anterior = archivo_seleccionado
        # ¡ELIMINADO! st.rerun() - Streamlit ya reinicia automáticamente

    # Mostrar información detallada del DataFrame
    if not df_base.empty:
        st.sidebar.success(f"✅ **{archivo_seleccionado}**")

        # Métricas principales
        col1, col2 = st.sidebar.columns(2)
        with col1:
            st.metric("📋 Filas", f"{len(df_base):,}")
        with col2:
            st.metric("📊 Columnas", len(df_base.columns))

        # Estructura de columnas
        with st.sidebar.expander("📋 Ver estructura"):
            st.write("**Columnas disponibles:**")
            for i, col in enumerate(df_base.columns):
                st.caption(f"{i+1}. {col}")

            st.write("**Tipos de datos:**")
            tipos = df_base.dtypes.value_counts()
            for tipo, count in tipos.items():
                st.caption(f"- {tipo}: {count} columnas")

        # Vista previa
        with st.sidebar.expander("👀 Vista previa (primeras 3 filas)"):
            st.dataframe(df_base.head(3), use_container_width=True)

        # Tamaño en memoria
        tamaño_mb = df_base.memory_usage(deep=True).sum() / (1024 * 1024)
        st.sidebar.caption(f"💾 Tamaño: {tamaño_mb:.2f} MB")

        # Mostrar valores únicos de columnas clave
        with st.sidebar.expander("🔍 Valores únicos en columnas clave"):
            columnas_clave = [
                "Periodo Meta",
                "Programa Academico",
                "Fuente Clasificada",
            ...
            for col in columnas_clave:
                if col in df_base.columns:
                    valores = df_base[col].dropna().unique()
                    if len(valores) <= 10:
                        st.caption(f"**{col}:** {', '.join(map(str, valores[:5]))}")
                    else:
                        st.caption(f"**{col}:** {len(valores)} valores únicos")
    else:
        st.sidebar.error("❌ DataFrame vacío")
    ...
    if st.sidebar.button("🔄 Recargar Datos", use_container_width=True):
        ...
        try:
            ...
            try:
                ...
                st.warning("⚠️ Por favor, refresca la página manualmente (F5)")
